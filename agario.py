...
def init(data):
    ...
    data.colors=['red','blue','yellow','white']
...
def timerFired(data):
  r= data.size*2 #20 is the initial me radius
  for ball in data.balls:
    x,y=ball[0]+data.offx,ball[1]+data.offy
    if getDistance(x,y,data.width/2,data.height/2)<=r:
      print(x,y)
      data.balls.remove(ball)
      data.balls.append((random.randint(-1000,1000),random.randint(-1000,1000)))
      data.size+=2
...
def drawMe(canvas,data):
  canvas.create_oval(data.width/2-data.size,
    data.height/2-data.size, data.size+data.width/2,
    data.size+data.height/2, fill = "red")
